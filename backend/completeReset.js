...
async function completeReset() {
  ...
  try {
    ...
    console.log('🧹 Complete reset of medicines and inventory...');

    // Truncate all related tables
    await client.query('TRUNCATE TABLE pharmacy_stocks CASCADE');
    console.log('✅ Truncated pharmacy_stocks');

    await client.query('TRUNCATE TABLE bincard CASCADE');
    console.log('✅ Truncated bincard');

    await client.query('TRUNCATE TABLE medicines CASCADE');
    console.log('✅ Truncated medicines');

    // Add only the 25 common medicines
    ...
    console.log(`\n🎉 Complete reset successful! Added ${commonMedicines.length} common medicines.`);

    // Final verification
    const finalCount = await client.query('SELECT COUNT(*) as total FROM medicines');
    console.log(`\n📊 Total medicines in database: ${finalCount.rows[0].total}`);
